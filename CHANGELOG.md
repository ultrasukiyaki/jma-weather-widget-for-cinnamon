...
- 設定画面とsettings schemaをv3.2.0と同じ構成へ復元
- 新しい設定項目を増やさず、防災通知を内部defaultで有効化
...
- Open-Meteo時間別予報の降水量、rain、showers、風向表示
- 気象庁の地域予報、風概況、元の時間帯単位の降水確率表示
- Provider別の有効時刻、発表時刻、鮮度表示
- 降水アイコン意味とProvider混在防止の自動テスト

### Changed

- 現在の推定天気と時間別予報をOpen-Meteoへ統一
- 時間別のアイコン、降水確率、降水量、風を同一レコードから表示
- 気象庁予報を現在・時間別とは独立した地域予報欄へ分離
- パネルの降水確率でJMA日次値へフォールバックしない

### Fixed

- 降水確率が高く予測雨量0 mmでも雨アイコンに見える可能性
- 地点予報と地域予報の風・天気が同じ粒度の値に見える問題

### Compatibility

- v3.1.1のUUID、設定キー、インスタンスID、キャッシュschema、保存パスを維持
...
- パネル表示用の現在時間別予報Resolver
- 日付変更、未整列・重複・不正日時、距離制限、Asia/Tokyo解釈の自動テスト

### Changed

- パネルの天気アイコンを現在時刻に対応する時間別予報へ同期
- パネルの降水確率をアイコンと同じ時間別予報レコードへ同期
- 現在気温、高温警告、雨・高温・UV通知、ポップアップ表示の既存データ経路を維持

### Fixed

- パネルと時間別予報で天気・降水確率が一致しない問題
- 毎時30分以降に現在時間の時間別行が候補から除外される問題
- システムタイムゾーンとOpen-MeteoのAsia/Tokyo日時を混同する可能性

### Compatibility

- v3.0.1のUUID、設定キー、インスタンスID、キャッシュschema、保存パスを維持
